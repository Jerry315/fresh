...
type Address struct {
	Id        int
	Receiver  string       `orm:"size(20)"`       // 收货人
	Addr      string       `orm:"size(50)"`       // 收货人地址
...
type Goods struct { // 商品SPU表
	...
	Detail   string      `orm:"size(255)"` // 商品详细描述
...
type IndexGoodsBanner struct { // 首页轮播商品展示
	Id       int
...
type IndexTypeGoodsBanner struct { // 首页分类商品展示表
	...
	GoodsSKU    *GoodsSKU  `orm:"rel(fk)"`    // 商品sku
	DisplayType int        `orm:"default(1)"` // 展示类型0代表文字，1代表图片
	Index       int        `orm:"default(0)"` // 展示顺序
}
...
func init() {
	// 获取连接对象
	orm.RegisterDataBase("default", "mysql", "root:123456@tcp(127.0.0.1:3306)/fresh?charset=utf8")

	orm.RegisterModel(new(User), new(Address), new(Goods), new(GoodsType), new(GoodsSKU), new(GoodsImage),
		new(IndexGoodsBanner), new(IndexPromotionBanner), new(IndexTypeGoodsBanner), new(OrderInfo), new(OrderGoods))
